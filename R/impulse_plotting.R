...
kinetics_plotting <- function(augmented_timecourses, saturation = 0.9, max_time, fit_timepoints = 100) {
  ...
  present_variables <- intersect(reserved_variables, colnames(augmented_timecourses))
  if (length(present_variables) == 0) {
    stop ("no aesthetic variables present; aesthetic variables are: ", paste(reserved_variables, collapse = ", "))
  }

  stopifnot(class(saturation) == "numeric", length(saturation) == 1, saturation > 0.5, saturation < 1)
  stopifnot(class(max_time) %in% c("numeric", "integer"), length(max_time) == 1, max_time > 0)
  ...
  if ("fitted_kinetics" %in% present_variables) {
    ...
    fitted_values <- fitted_kinetics %>%
      tidyr::nest(rate, t_rise, t_fall, v_inter, v_final, .key = "parameters") %>%
      dplyr::mutate(fitted_timecourses = purrr::map2(parameters, model, fit_timecourse, timepts = timepoints)) %>%
      tidyr::unnest(fitted_timecourses) %>%
      dplyr::mutate(best_model = ifelse(model == best_model, TRUE, FALSE))

    # calculate saturation interval

    kinetic_intervals <- kinetic_aesthetics(fitted_kinetics %>%
                                              dplyr::filter(model == best_model),
                                            fitted_values, saturation = saturation)

    kinetics_plot <- kinetics_plot +
      geom_rect(data = kinetic_intervals$asympote_aesthetics_df, aes(xmin = t_saturation_start, xmax = t_saturation_end), ymin = -Inf, ymax = Inf, alpha = 0.25) +
      geom_hline(yintercept = 0, color = "BLACK", size = 1) +
      geom_path(data = fitted_values, aes(x = time, y = fit, color = model, group = model, linetype = best_model)) +
      # timing
      geom_segment(data = kinetic_intervals$time_aesthetics_df, aes(x = time, y = fit - 0.25, xend = time, yend = fit + 0.25), color = "limegreen", size = 2) +
      geom_text(data = kinetic_intervals$time_aesthetics_df %>% dplyr::filter(par == "t_rise"), aes(x = time + 2, y = fit), label = "t[rise]", parse = TRUE, hjust = 0, size = 4) +
      geom_text(data = kinetic_intervals$time_aesthetics_df %>% dplyr::filter(par == "t_fall"), aes(x = time + 2, y = fit), label = "t[fall]", parse = TRUE, hjust = 0, size = 4) +
      # assymptotes
      geom_segment(data = kinetic_intervals$asympote_aesthetics_df, aes(x = t_saturation_end - 5, y = assymp, xend = t_saturation_end + 5, yend = assymp), color = "limegreen", size = 2) +
...
kinetic_aesthetics <- function(fitted_kinetics, fitted_values, saturation = 0.9) {
  ...
  asympote_aesthetics_df <- fitted_kinetics %>%
    ...
    dplyr::mutate(t_saturation_start = saturation_time(1 - saturation, value, rate),
                  t_saturation_end = saturation_time(saturation, value, rate)) %>%
    dplyr::select(tc_id, model, assymp_type, assymp, t_saturation_start, t_saturation_end)
