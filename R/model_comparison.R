...
impulse_sigmoid_comparison <- function(timecourse_losses, fdr_cutoff = 0.001) {
  ...
  missing_required_colnames <- setdiff(required_colnames,
                                       colnames(timecourse_losses))
  if (length(missing_required_colnames) != 0) {
    stop ("\"timecourse_losses\" is missing required variables: ",
          paste(missing_required_colnames, collapse = ", "))
  }
  ...
  missing_required_vars_spread_models <- setdiff(required_vars_spread_models,
                                                 colnames(spread_models))
  if (length(missing_required_vars_spread_models) != 0) {
    stop ("\"timecourse_losses\" is missing required models: ",
          paste(missing_required_vars_spread_models, collapse = ", "))
  }
  ...
  if (nrow(incomplete_comparisons) != 0) {
    warning (nrow(incomplete_comparisons),
             " tc_ids do not contain both a sigmoid and impulse model")
  }
  ...
  spread_models %>%
    dplyr::mutate(logLik_diff = impulse - sigmoid,
                  ...
                  model_pchisq = stats::pchisq(logLik_diff,
                                               df = 2,
                                               lower.tail = FALSE),
                  # correct using Benjamini-Hochberg (using this instead of
                  # Storey because there is an enrichment of p-values near 1
                  # due to timecourses with strong prior constraints which
                  # were fit much better with a sigmoid than an impulse
