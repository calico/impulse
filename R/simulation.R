...
simulate_timecourses <- function (n, Pr_impulse = 0.5, timepts = seq(0, 120, by = 5),
  ...
  model_counts <- stats::rmultinom(1, size = n, prob = c(1-Pr_impulse, Pr_impulse))
...
add_noise <- function(measurements, measurement_sd) {
  measurements %>%
    dplyr::mutate(abundance = stats::rnorm(dplyr::n(), mean = sim_fit, sd = measurement_sd))
}
...
simulate_parameters <- function (n, model = "sigmoid",  prior_pars = c("v_sd" = 1.2, "rate_shape" = 2, "rate_scale" = 0.25, "time_shape" = 5, "time_scale" = 5)) {
